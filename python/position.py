...
from remove_name import Write_remove_name
##from cnc import cnc1
# 3-fold repetition
# 50-move rule
...
class Position():
    ...
    def make_move(self, move) -> MoveResult:
        ...
        if move.is_capture:
            self.halfmove_clock = 0
            print("EMA SHELH"+" "+str(move.from_sq)+str(move.to_sq))
            dup1 = copy.copy(self.piece_map)##Copy location
            file_loc = open("remove.txt", "w+")
            file_loc.writelines(Write_remove_name(dup1,move.to_sq))
            ...
            ##cnc1(int(move.from_sq),int(move.to_sq))
            ##REMOVE OBJECT
        ...
        if self.is_en_passant_capture:
            if move.color == Color.WHITE:
                ...
                dup1 = copy.copy(self.piece_map)##Copy location
                file_loc = open("remove.txt", "w+")
                file_loc.writelines(Write_remove_name(dup1,move.to_sq - 8))
                file_loc.close()  # to change file access modes
        ...
        ##MOVE PICE TO NEW LOACTION
        ##cnc1(int(move.from_sq), int(move.to_sq))
        file1 = open("x.txt", "w")
        file2 = open("y.txt", "w")

        Xcor = [str(move.from_sq)]
        Ycor = [str(move.to_sq)]
        file1.writelines(Xcor)
        file2.writelines(Ycor)
